...
Consistency comes from pasting the block below instead. Duplicating ~30 lines of CSS seventeen times is
...
from `defence-in-depth`), so the two collections read as one family.
...
- **Verdict colours are fixed**: `--good` green, `--warn` amber, `--bad` red. Don't re-map them per
  toy; a red band must mean the same thing in all seventeen.
...
consistent across the collection (and consistent with security-toys):

| Role | Token | Colour |
|---|---|---|
| What the user controls | `--user` | `#2dd4bf` teal |
| What the system/model controls | `--system` | `#a78bfa` purple |
| What the attacker controls | `--attacker` | `#f87171` red |

`data-or-command` leans on all three: the prompt is teal, the summariser purple, the planted
instruction red. A student who has played the security toys reads the palette the same way here.
